...
pub fn solve19(input: &[String]) -> Solutions {
    let towels: HashSet<&str> = input[0].split(", ").collect();
    ...
    let (total_designs_possible, total_num_ways) = designs
        .iter()
        .map(|design| count_ways(&mut cache, &towels, max_towel_len, design))
        .fold((0, 0), |(possible_designs, num_ways), this_design_ways| {
            (
                possible_designs + (this_design_ways > 0) as i32,
                num_ways + this_design_ways,
            )
        });

    (
        Solution::I32(total_designs_possible),
